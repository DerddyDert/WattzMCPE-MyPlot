...
class MYSQLDataProvider extends DataProvider {
	...
	public function __construct(MyPlot $plugin, $cacheSize = 0) {
		...
		$sql = "SELECT id, name, owner, helpers, biome, X, Z, level FROM plots WHERE owner = ?;";
		$this->checkPreparedStatement ( $queryName, $sql );
		
		$queryName = "getPlotsByOwnerAndLevel"; // ss
		$sql = "SELECT id, name, owner, helpers, biome, X, Z, level  FROM plots WHERE owner = ? AND level = ?;";
		$this->checkPreparedStatement ( $queryName, $sql );
	...
	public function savePlot(Plot $plot) {
		...
		if ($plot->id >= 0) {
			...
			$bind_result = $this->db_statements [$thisQueryName]->bind_param ( "ssssi", $plot->name, $plot->owner, $helpers, $plot->biome, $plot->id );
		} else {
			$thisQueryName = "SavePlot";
			$bind_result = $this->db_statements [$thisQueryName]->bind_param ( "siisiissss", $plot->levelName, $plot->X, $plot->Z, $plot->levelName, $plot->X, $plot->Z, $plot->name, $plot->owner, $helpers, $plot->biome );
		}
	...
	public function deletePlot(Plot $plot) {
		if ($plot->id >= 0) {
			...
			$bind_result = $this->db_statements [$thisQueryName]->bind_param ( "i", $plot->id );
		} else {
			$thisQueryName = "RemovePlot";
			$bind_result = $this->db_statements [$thisQueryName]->bind_param ( "sii", $plot->levelName, $plot->X, $plot->Z );
		}
	...
	public function getPlot($levelName, $X, $Z) {
		...
		$bind_result = $this->db_statements [$thisQueryName]->bind_param ( "sii", $levelName, $X, $Z );
		if ($bind_result === false) {
			$err = $this->db_statements [$thisQueryName]->error;
			$this->criticalError ( "Could not bind to query " . $thisQueryName . ": " . $err );
			return false;
		...
		$result = $this->db_statements [$thisQueryName]->bind_result ( $id, $name, $owner, $helpers_csv, $biome );
		if ($result === false) {
			$err = $this->db_statements [$thisQueryName]->error;
		...
		if ($this->db_statements [$thisQueryName]->fetch ()) {
			...
			$plot = new Plot ( $levelName, $X, $Z, ( string ) $name, $owner, $helpers, ( string ) $biome, ( int ) $id );
		} else {
	...
	public function getPlotsByOwner($owner, $levelName = "") {
		...
		$result = $this->db_statements [$thisQueryName]->bind_result ( $id, $name, $owner, $helpers, $biome, $X, $Z, $foundlevel );
		if ($result === false) {
			$err = $this->db_statements [$thisQueryName]->error;
			$this->criticalError ( "Failed to bind result " . $thisQueryName . ": " . $err );
			return false;
		...
		while ( $this->db_statements [$thisQueryName]->fetch () ) {
			...
			$plots [] = new Plot ( $foundlevel, ( int ) $X, ( int ) $Z, ( string ) $name, $owner, $helpers, ( string ) $biome, ( int ) $id );
		}
